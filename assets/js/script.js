...
var weatherDet = document.querySelector("#city-weather");
...
function fetchWeather(coord) {
  ...
  fetch(apiUrl)
    ...
    .then(data => {
      ...
      $("#city-weather").append(cityName, cityTemp, cityWind, cityHumid);
...
function handleSearchForm(e) {
  ...
  if (!recentCitySearchArr.includes(cityName)) {
    ...
    var searchedCity = $(`
            <li class="list-group-item list-city">${cityName}</li>
        `);
...
// function handleSearchHistory(recentCitySearchArr) {
//   if (recentCitySearchArr !== null) {
//     var lastSearchedIndex = recentCitySearchArr.length - 1;
//     var lastSearchedCity = recentCitySearchArr[lastSearchedIndex];
//     currentCondition(lastSearchedCity);
//     console.log(`Last searched city: ${lastSearchedCity}`);
//   }
// }

$(document).on("click", recentCitySearchArr, function () {
  var listCity = $(this).text();
  fetchWeather(listCity);
});

// $(document).ready(function () {
//   var recentCitySearchArr = JSON.parse(localStorage.getItem("city"));
